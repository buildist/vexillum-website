...
{
	global $pager;
	$pager[$id]['count'] = mysql_num_rows(db_query($query));
...
function pager_pagelinks($id)
{
	...
	if($pager[$id]['stop'] > $pager[$id]['items_per_page'])
		echo '<a class="linkbutton" href="'.(self(array('start'.pager_suffix($id) => ($pager[$id]['start'] - $pager[$id]['items_per_page'])))).'">&laquo; Previous Page</a>';
	if($pager[$id]['stop'] < $pager[$id]['count'])
		echo ' <a class="linkbutton" href="'.(self(array('start'.pager_suffix($id) => $pager[$id]['stop']))).'">Next Page &raquo;</a>';
}
?>